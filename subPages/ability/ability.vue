...
<script>
import {getAbilityList, getItemsList, getSkillList} from "../../server";
...
export default {
  ...
  methods: {
    to(item) {
      uni.navigateTo({
        url: '/subPages/abilityDetail/abilityDetail?id=' + item.id,
      })
    },
    next() {
      this.page++;
    ...
    getData() {
      this.status = 'loading';
      getAbilityList({page: this.page, per_page: this.pageSize, name: this.searchValue}).then(res => {
        console.log(res)
